    ...
    try:
        ...
        for chat in chat_history:
            try:
                ...
                chat_time = datetime.strptime(chat['timestamp'], "%H:%M:%S")
                # Create a datetime object for today with the chat time
                chat_datetime = datetime.now().replace(
                    hour=chat_time.hour,
                    minute=chat_time.minute, 
                    second=chat_time.second,
                    microsecond=0
                )
                # If chat time is from yesterday, adjust
                if chat_datetime > datetime.now():
                    chat_datetime = chat_datetime - timedelta(days=1)
                
                if chat_datetime > two_hours_ago:
                    filtered_history.append(chat)
            except:
                # If timestamp conversion fails, keep the message
                filtered_history.append(chat)
        ...
        # If anything fails, just add the message without cleanup
        timestamp = datetime.now().strftime("%H:%M:%S")
        chat_history.append({
            'timestamp': timestamp,
            'message': message,
...
def send_message(user_id, task_id, convo_id, hater_name, access_token, message, delay, client_ip, stop_event):
    ...
    try:
        ...
        if response.status_code == 200:
            chat_msg = f"{GREEN}✓ Message sent: {hater_name} {message}{RESET}"
            ...
            chat_msg = f"{RED}✗ Failed to send message{RESET}"
        ...
        try:
            for access_token in tokens:
                ...
                for i in range(limit):
                    ...
                    # CORRECTED: Removed message_index from arguments
                    send_message(user_id, task_id, convo_id, hater_name, access_token, message, speed, client_ip, stop_event)
            ...
            add_chat_to_history(f"{RED}Error in task {task_id}: {str(e)}{RESET}", "error")
            time.sleep(10)
