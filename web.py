...
task_configs = {}  # Store task configurations for restarting
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {% else %}
                            <form method="post" action="{{ url_for('start_task', task_id=tid) }}" style="margin: 0;">
...
def index():
    global tasks, accounts, stop_keys, task_configs
...
def start_task(task_id):
    global tasks, task_stop_events, task_threads, task_configs, stop_keys
    ...
    if task_id in task_configs:
        ...
        # Generate new stop key
        stop_key = generate_stop_key()
        stop_keys[task_id] = stop_key
        
        tasks[task_id] = {
            'status': 'Starting',
            'tokens_count': len(config['tokens']),
            'messages_sent': 0
        }
        ...
        thread = threading.Thread(
            target=send_messages, 
            args=(task_id, config['convo_id'], config['hater_name'], 
                  config['tokens'], config['messages'], config['speed'], 
                  config['limit'], config['client_ip'], stop_event)
        )
        task_threads[task_id] = thread
        thread.daemon = True
        thread.start()
        
        add_chat_to_history(f"{GREEN}Task {task_id} restarted with new stop key: {stop_key}{RESET}", "success")
    else:
        add_chat_to_history(f"{RED}Task {task_id} configuration not found{RESET}", "error")
